"""
Versão de produção da app.py
Ajustada para rodar em Heroku/Railway/Render
"""
import os
import json
...
def obter_analise_atual():
    ...
    if sinais:
        try:
            ...
            print(f"[WARN] Erro ao analisar com Gemini: {e}")
...
def agendar_backtest():
    ...
    try:
        ...
        print(f"[WARN] Scheduler não disponível em produção: {e}")
...
def health_check():
    """Health check para Railways/Render"""
    return jsonify({
        "status": "ok",
        "timestamp": agora_brt().isoformat()
    })
...
if __name__ == '__main__':
    ...
    # Executar backtest inicial em thread separada
    print("[INIT] Executando backtest inicial...")
    thread = threading.Thread(target=executar_backtest_async)
    thread.daemon = True
    thread.start()
    
    # Pegar porta do ambiente ou usar 5000 como padrão
    port = int(os.environ.get('PORT', 5000))
    debug = os.environ.get('DEBUG', 'False') == 'True'
    
    # Iniciar servidor Flask
    print(f"[SERVER] Iniciando em http://0.0.0.0:{port}")
